...
interface InstrumentFamiliesResponse {
  success: boolean;
  code: number;
  message: string;
...
interface InstrumentFamilyResponse {
  success: boolean;
  code: number;
  message: string;
  data: InstrumentFamily;
...
export default function InstrumentService() {
  ...
  const {
    ...
    queryFn: async (): Promise<InstrumentFamily[]> => {
      ...
      const data: InstrumentFamiliesResponse = await response.json();
      return data.data;
    },
  });
  ...
  const mutation = useMutation({
    mutationFn: async (newServiceTypes: string[]) => {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/instrument-family/${selectedFamilyId}/type/${selectedTypeId}`,
        {
          method: "PUT",
      ...
      const data: InstrumentFamilyResponse = await response.json();
      return data;
    },
    onSuccess: () => {
  ...
  const handleSubmit = () => {
    ...
    if (!selectedFamilyId || !selectedTypeId)
          ...
          <select
            ...
            onChange={(e) => {
              ...
              const selectedInstrumentType =
                selectedFamily?.instrumentTypes.find((t) => t._id === typeId);
          ...
          >
            ...
            {instrumentFamilies.map((family) =>
              family.instrumentTypes.map((type) => (
                <option
                  key={type._id}
                  value={`${family._id}||${type._id}||${type.type}`}
                >
                  {type.type}
                </option>
              )),
